...
/* Prototipos */
_pNodo Cuenta(tipoNodo* Lista, char c);
_pNodo Ordenar(tipoNodo* Lista);
_pNodo InsertarOrden(tipoNodo* Cabeza, tipoNodo *e);
...
{
	clock_t start = clock();/*Inicia el cronometro para medir el tiempo*/
...
printf(".") ;  
   ...
   fe = fopen("mensaje.txt", "r");
   while((c = fgetc(fe)) != EOF)
   {
      Longitud++;       /* Actualiza la cuenta de la longitud del fichero */
      Lista=Cuenta(Lista, c); /* Actualiza la lista de frecuencias */
   }
   fclose(fe);
printf(".") ;  
...
printf("Tiempo transcurrido: %fseg", ((double)clock() - start) / CLOCKS_PER_SEC);/*tiempo transcurrido*/
   system("Pause");
   return 0;
}
...
_pNodo Cuenta(tipoNodo* Lista, char c)
{
   ...
   if(Lista==NULL)  /* Si la lista está vacía, el nuevo nodo será Lista */
   {
...
_pNodo Ordenar(tipoNodo* Lista)
{
   ...
   while(Lista2)
   {
      ...
      Lista2 = a->sig;
      Lista=InsertarOrden(Lista, a); /* Y los inserta por orden en Lista */
   }
   return Lista;
}
